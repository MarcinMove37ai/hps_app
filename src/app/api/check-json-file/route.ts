// src/app/api/check-json-file/route.ts
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Utwórz ścieżkę docelową w podfolderze json
    const baseName = fileName.replace(/\.pdf$/, '');
    const jsonFileName = `json/${baseName}.json`;

    console.log(`Sprawdzanie czy plik ${jsonFileName} istnieje w buckecie ${process.env.S3_BUCKET_NAME || 'ebooks-in'}`);
    ...
    try {
      ...
      return NextResponse.json({
        exists: true,
        jsonFileName,
        message: 'Plik JSON został znaleziony'
      });
    } catch (error) {
      ...
      if (s3Error.name === 'NotFound') {
        return NextResponse.json({
          exists: false,
          message: 'Plik JSON jeszcze nie istnieje, przetwarzanie trwa'
        });
      }
