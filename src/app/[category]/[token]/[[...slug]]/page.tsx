// src/app/[category]/[token]/[[...slug]]/page.tsx
...
interface PageParams {
  category: string; // Dodana kategoria jako parametr URL
  token: string;
  slug?: string[];
}

// Funkcja do pobierania danych strony z bazy danych
async function getPageData(categoryParam: string, tokenParam: string) {
  let client;

  try {
    client = await pool.connect();

    // Pobieramy wszystkie pola z tabeli pages, zmodyfikowane zapytanie uwzględnia kategorię
    const result = await client.query(
      `SELECT * FROM pages WHERE url LIKE $1 AND status = 'active'`,
      [`%/${categoryParam}/${tokenParam}%`]
    );
    ...
    console.log('DEBUG - Dane pobrane z bazy dla SEO:', {
      ...
      category: result.rows[0].category // Dodajemy kategorię do logów
    });
...
export async function generateMetadata(
  ...
  try {
    ...
    const { category, token } = resolvedParams;

    // Pobranie danych strony z uwzględnieniem kategorii
    const pageData = await getPageData(category, token);
    ...
    console.log('DEBUG - Metadane używane do SEO:', {
      ...
      category: pageData.category // Dodajemy kategorię do logów
    });
